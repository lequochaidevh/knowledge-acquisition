...
void PipeServer::ProcessIncomingTL() {
    PacketHeader         header;
    std::vector<uint8_t> data;

    // 1. Receive from PosixPipe
    if (Receive(read_fd, header, data)) {
        CalculateHz(header.timestamp_ms);
        ...
        if (mode == ServerMode::Feedback) {
            ...
            Send(write_fd, DataType::Command, empty_ack, header.sequence_id);
        }
    }
}
