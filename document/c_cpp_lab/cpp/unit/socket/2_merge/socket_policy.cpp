...
class SocketDgramIPv4Policy {
 public:
    ...
    static void init(int& rx_fd, int& tx_fd, Context& ctx) {
        rx_fd = ::socket(AF_INET, SOCK_DGRAM, IPPROTO_UDP);
        tx_fd = ::socket(AF_INET, SOCK_DGRAM, IPPROTO_UDP);
        if (rx_fd == -1 || tx_fd == -1) {
        ...
        ::bind(rx_fd, (sockaddr*)&ctx.local_sockaddr, sizeof(ctx.local_sockaddr));
        ...
        ::connect(tx_fd, (sockaddr*)&ctx.target_sockaddr, sizeof(ctx.target_sockaddr));
        std::cout << "[Policy IPv4 DGRAM] Channels initialized." << std::endl;
    }
...
class SocketDgramPathPolicy {
 public:
    ...
    static void init(int& rx_fd, int& tx_fd, Context& ctx) {
        ...
        std::strncpy(ctx.target_sockaddr.sun_path, ctx.target_path.c_str(), sizeof(ctx.target_sockaddr.sun_path) - 1);
        ::connect(tx_fd, (sockaddr*)&ctx.target_sockaddr, sizeof(ctx.target_sockaddr));
        std::cout << "[Policy Unix DGRAM] File nodes initialized." << std::endl;
    }
...
class SocketStreamPathPolicy {
 public:
    ...
    static void init(int& rx_fd, int& tx_fd, Context& ctx) {
        rx_fd = ::socket(AF_UNIX, SOCK_STREAM, 0);
        tx_fd = ::socket(AF_UNIX, SOCK_STREAM, 0);

        if (rx_fd == -1 || tx_fd == -1) {
            throw std::runtime_error("Unix STREAM Socket creation failed");
        }
        ...
        std::cout << "[Policy Unix STREAM] Stream pipeline nodes initialized successfully." << std::endl;
    }
...
class FlexibleSocket {
 private:
    int _tx_fd = -1;
    int _rx_fd = -1;

    // Automatically extracts the concrete context type associated with the Policy at compile-time!
    // (Tự động trích xuất kiểu dữ liệu Context thực tế từ Policy thông qua từ khóa using)
    ...
    void receive_loop() {
        std::cout << "[FlexibleSocket] Async reader runtime queue loop active.\n";
        ...
        while (_is_running) {
            ...
            if (received < 0) {
                ...
                std::cerr << "[FlexibleSocket Error] Read execution failed.\n";
                break;
            }
            ...
            if (received >= static_cast<ssize_t>(sizeof(PacketHeader))) {
                ...
                std::cout << "[FlexibleSocket Receiver] Decoded -> Type: " << header.data_type
                          << " | Seq: " << header.seq << " | Size: " << header.payload_size << "\n"
                          << "[FlexibleSocket Receiver] Content: " << content << "\n";
            }
        }
    }
 ...
 public:
    ...
    FlexibleSocket(const typename Policy::Context& context) : _ctx(context) {
        // Delegate all hardware allocation and memory setups to the isolated static class policy
        Policy::init(_rx_fd, _tx_fd, _ctx);

        // Fire background thread worker tasks
        _receive_thread = std::thread(&FlexibleSocket::receive_loop, this);
    }
    ...
    ~FlexibleSocket() {
        ...
        Policy::cleanup(_ctx);
        std::cout << "[FlexibleSocket] Scope closed cleanly. Resources completely recycled.\n";
    }
...
int main() {
    ...
    std::cout << "--- 1. INSTANTIATING SOCKET VIA IPV4 DGRAM POLICY ---\n";
    {
        ...
        FlexibleSocket<SocketDgramIPv4Policy> ip_node(ip_ctx);
        FlexibleSocket<SocketDgramIPv4Policy> ip2_node(ip2_ctx);
    ...
    std::cout << "\n--- 2. INSTANTIATING SOCKET VIA UNIX DOMAIN PATH POLICY ---\n";
    {
        ...
        FlexibleSocket<SocketDgramPathPolicy> unix_node(unix_ctx);
        FlexibleSocket<SocketDgramPathPolicy> unix2_node(unix2_ctx);

        for (int i = 0; i < 20; i++) {
            std::string combine  = std::to_string(i) + " " + payload;
            std::string combine2 = std::to_string(i) + " " + payload2;
            unix_node.send_packet(99, combine, 666);
            std::this_thread::sleep_for(std::chrono::milliseconds(300));
            unix2_node.send_packet(123, combine2, 888);
            std::this_thread::sleep_for(std::chrono::milliseconds(300));
        }
